...
(function () {
  ...
  window.selectOption = function (btn) {
    ...
    // Multi-select is detected if the correct string contains a comma
    const isMulti = correctVal.includes(',');
  ...
  window.checkQuiz = function (containerId) {
    ...
    const questionBlocks = $$('.quiz-question-block', container);
    
    questionBlocks.forEach(q => {
      const allOptions = $$('.quiz-option', q);
      const selectedNodes = $$('.quiz-option.selected', q);
      const feedback = $('.quiz-feedback', q);
      const correctStr = q.dataset.correct || '';
      const rightExp = q.dataset.explanationRight || '';
      const wrongExp = q.dataset.explanationWrong || '';

      // Clear previous states
      allOptions.forEach(o => o.classList.remove('correct', 'incorrect'));
      ...
      const correctValues = correctStr.split(',').map(v => v.trim());
      const selectedValues = selectedNodes.map(n => n.dataset.value);
      ...
      allOptions.forEach(opt => {
        ...
        if (isCorrect) {
          ...
          if (!isSelected) isFullyCorrect = false;
        } else {
          if (isSelected) {
      ...
      // Final check for missed multi-select items
      if (isFullyCorrect && selectedValues.length !== correctValues.length) {
        isFullyCorrect = false;
      }

      if (isFullyCorrect) {
        feedback.innerHTML = '<strong>Correct!</strong> ' + rightExp;
